...
// Garante que apenas o Administrador acesse (Requisito 1.6)
...
app.get('/home', verificarAutenticacao, (req, res) => res.render('index', { usuario: req.session.usuario }));
app.get('/logout', (req, res) => { req.session.destroy(); res.redirect('/login'); });

// --- ÁREA DO ADMINISTRADOR (REQUISITO 1.6) ---
app.get('/admin', verificarAdmin, (req, res) => res.render('admin/dashboard', { usuario: req.session.usuario }));
...
// Gerenciar Categorias e Habilidades
...
app.get('/receitas', verificarAutenticacao, async (req, res) => {
    // Aluno vê apenas as receitas que ele é responsável (ou Admin vê tudo)
...
app.post('/receitas', verificarAutenticacao, upload.single('imagem'), async (req, res) => {
    ...
    try {
        ...
        // Relacionamento N:N Autores (Requisito 1.3)
        // Garante que o autor logado sempre está vinculado
...
app.get('/receitas/excluir/:id', verificarAutenticacao, async (req, res) => {
    // Apenas responsáveis podem excluir (Requisito 1.5)
...
app.post('/habilidades', verificarAutenticacao, async (req, res) => {
    ...
    // Verifica se aluno já tem a habilidade para dar UPDATE ou INSERT
